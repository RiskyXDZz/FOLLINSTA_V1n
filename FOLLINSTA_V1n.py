...
class KIRIMKAN:
    ...
    def PENGIKUT(self, session, username, password, host, your_username):
        ...
        if self.ANTI_FORGERY_TOKEN != None:
            ...
            if '\'status\': \'success\'' in str(self.JSON_RESPONSE):
                ...
                if 'name="userID"' in str(response3.text):
                    ...
                    if '\'status\': \'success\'' in str(self.JSON_RESPONSE4):
                        ...
                        time.sleep(4.5)
                    printf(f"[bold bright_black]   ──>[bold green] BERHASIL KONTOL {str(host).split('.')[0].upper()} SERVICE!           ", end='\r')
                    time.sleep(5.0)
                    return (True)
                else:
                    printf(f"[bold bright_black]   ──>[bold red] TARGET USERNAME TIDAK ADAD!           ", end='\r')
                    time.sleep(4.5)
                    return (False)
            elif 'Güvenliksiz giriş tespit edildi.' in str(self.JSON_RESPONSE):
                CHECKPOINT.append(f'{self.JSON_RESPONSE}')
                printf(f"[bold bright_black]   ──>[bold red] AKUN TERKENA CHECKPOINT!          ", end='\r')
                time.sleep(4.5)
                return (False)
            elif 'Üzgünüz, şifren yanlıştı.' in str(self.JSON_RESPONSE):
                BAD.append(f'{self.JSON_RESPONSE}')
                printf(f"[bold bright_black]   ──>[bold red] PASSWORD BENAR!              ", end='\r')
                time.sleep(4.5)
                return (False)
            else:
            ...
            printf(f"[bold bright_black]   ──>[bold red] TOKEN BERMASALAH!          ", end='\r')
            time.sleep(2.5)
            return (False)
...
class MAIN:
    ...
    def __init__(self):
        ...
        try:
            self.LOGO()
            printf(Panel(f"[bold white]masukan username instagram mu dan pisahkan passwordnya dengan tanda `[bold green]:[bold yellow]` NOTE\n, Jangan Takut ini gak phising", width=59, style="bold bright_black", title="[bold bright_black][Login Diperlukan]", subtitle="[bold bright_black]╭──────", subtitle_align="left"))
            ...
            if ':' in str(self.ACCOUNTS):
                ...
                printf(Panel(f"[bold white] Masukan username kembali,gak usah buat passwordnya hanya usernamee\n. Contoh username:[bold green] kyy.sh21", width=59, style="bold bright_black", title="[bold bright_black][Username]", subtitle="[bold bright_black]╭──────", subtitle_align="left"))
                self.YOUR_USERNAME = Console().input("[bold bright_black]   ╰─> ").replace('@', '')
                if len(self.YOUR_USERNAME) != 0:
                    printf(Panel(f"[bold white]Tekan [bold yellow] CTRL + C[bold white] Untuk berhenti[bold red] CTRL + Z[bold white] Untuk keluar dari sc,\nMasuk ke akun instagramnya setiap hack Followernya mencapai berhasil 4X.. (NURUT AJA KONTOL BIAR MAKIN BANYAK)", width=59, style="bold bright_black", title="[bold bright_black][Catatan]"))
                    while (True):
                        try:
...
[bold white]Jumlah :[bold yellow] {self.JUMLAH}""", width=59, style="bold bright_black", title="[bold bright_black][Sukses]"))
    ...
    def LOGO(self):
        ...
        printf(Panel(r"""[bold red] _____ ___  _     _     ___ _   _ ____ _____  _
|  ___/ _ \| |   | |   |_ _| \ | / ___|_   _|/ \
| |_ | | | | |   | |    | ||  \| \___ \ | | / _ \
[bold white]|  _|| |_| | |___| |___ | || |\  |___) || |/ ___ \
|_|   \___/|_____|_____|___|_| \_|____/ |_/_/   \_\        [underline yellow]Gratis Follow instagram -By  RiskyXD,, Follow github Gw.!""", width=59, style="bold bright_black"))
        return (True)
    ...
    def DELAY(self, menit, detik, your_username):
        ...
        while (self.TOTAL):
            ...
            printf(f"[bold bright_black]   ──>[bold red] @{str(your_username)[:20].upper()}[bold white]/[bold green]{MENIT:02d}:{DETIK:02d}[bold white] SUKSES:-[bold green]{len(SUKSES)}[bold white] GAGAL:-[bold red]{len(GAGAL)}     ", end='\r')
            time.sleep(1)
            self.TOTAL -= 1
        return (True)
